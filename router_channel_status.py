...
def BrigdedMode():
    ...
    i = 0
    j = 0
    soup = BeautifulSoup(bridged.content, 'html5lib')
...
def RouterMode():
    ...
    i = 0
    j = 0
    routed = requests.get('http://192.168.0.1/Docsis_system.asp')
    soup = BeautifulSoup(routed.content, 'html5lib')
    table = soup.find('table', {'summary': 'Downstream Channels'})
    cols_pwr = table.findAll('td', {'headers': 'ch_pwr'})
    cols_snr = table.findAll('td', {'headers': 'ch_snr'})
...
bridged = requests.get('http://192.168.100.1/Docsis_system.asp')
